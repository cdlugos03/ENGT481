...
[Kest,L,P,M,Z] = kalmd(lsys,0.1,0.1,Ts);
...
Yest = Y + randn(4,1)*0.0001; %initialize variable for kalman filter estimation

Yn = [0;0;0;0]; %initialize array for state noise

Ymeas = [0;0;0]; %array for measurements
...
kint = 0.3; %integral gain
xint = 0; %set initial xint value
kprop = 0.5; %set proportional gain
kdiff = 0.6; %derivative gain
...
for k = 1:N %run from time step 1 to time step N
    ...
    Ylast = Yest; %store previous states

    %kalman filter
    Yest = lsys.A * Ylast + lsys.B * u; %predict state derivatives
    Yest = Ylast + Yest * Ts; %add derivatives *Ts to the previous states
    
    %estimate dtheta from theta measurements - NEED TO IMPROVE
    dtheta = (((0.01*Yn(1) + 1.99*Yest(1))/2) - Ylast(1)) / (Ts);
    Yn = [Yn(1);dtheta;Yn(3);Yn(4)]; %substitute in estimated dtheta

    Yest = Yest + L * (Yn - lsys.C * Yest); %factor in measured states
    % Yest = Yn; %test without kalman filter

    %compute derivative and integral of x
    xint = xint - Yest(3) * Ts; %approximate integral of x
    xdiff = (xlast - Yest(3))/Ts; %aproximate derivative of x
    xlast = Yest(3); %store x value for next loop
    
    %compute control input, and correct for x error
    %u = 0; %test open loop
    u = -Kd * Yest; %calculate control input
    u = u - kint * xint + kprop * Yest(3) - kdiff * xdiff; %correct for drift with integral/derivative of x (Y(3))
...
plot(t, Yest_store(2,:), 'DisplayName', 'd\theta/2dt (rad/s)');
