...
for k = 1:N %run from time step 1 to time step N
    ...
    %START CONTROL LOOP
    Ymeas = [Yn(1);Yn(3);Yn(4)]; %[theta, x, dx/dt]

    %store previous states
    Ylast = Yest; %store previous states

    %kalman filter - Initialize Yest with raw measured states, and dtheta=0
    Yest = lsys.A * Ylast + lsys.B * u; %predict state derivatives
    Yest = Ylast + Yest * Ts; %add derivatives *Ts to the previous states

    Yest = Yest + L * (Ymeas - Ksys.C * Yest); %factor in measured states
    ...
    u = u - kint * xint + kprop * Yest(3) - kdiff * Yest(4); %correct for drift with integral/derivative of x (Y(3))
    ...
    v = Ymeas(3) + a * Ts; %calculate target velocity of cart
    ...
    %add noise to states
    Yn = Y + randn(4,1)*0.000384; %max 0.0.022° error

    %store states and control input at each time step for plotting response
    Y_store(:,k+1) = min(max(Yn,-10), 10);
    Yest_store(:,k+1) = min(max(Yest,-10), 10);
    U_store(k+1) = min(max(u,-10), 10);
    f_store(k+1) = fpulse/1000; %store pulse frequency in kHz
